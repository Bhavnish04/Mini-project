...
def draw_window(red,yellow,red_bullets, yellow_bullets, red_health, yellow_health):
    WIN.blit(SPACE, (0, 0))
    WIN.fill(WHITE)       #make background white
    ...
    WIN.blit(YELLow_spaceship , (yellow.x, yellow.y)) #put the spaceship on the surface
...
def handle_bullets(yellow_bullets, red_bullets, yellow, red):  #move the bullets, remove the bullets or handle the collison
    for bullet in yellow_bullets:
        bullet.x += BULLET_VEL
...
def main():
    red = pygame.Rect(700, 300, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)  #coordinates
    yellow = pygame.Rect(100, 300, SPACESHIP_WIDTH, SPACESHIP_HEIGHT) #coordinates
    ...
    clock=pygame.time.Clock()    #times "run " loop will run per second
    run=True    
    while run:
        clock.tick(FPS)
